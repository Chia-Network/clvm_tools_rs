...
pub fn map_err_to_pyerr(
    srcloc: Srcloc,
    r: PyResult<Py<PyAny>>
) -> Result<Py<PyAny>, RunFailure> {
    r.map_err(|e| {
        RunFailure::RunErr(srcloc, format!("{}", e))
    })
}

pub fn python_value_to_clvm(
    py: Python, val: Py<PyAny>
) -> Result<Rc<SExp>, RunFailure> {
    let srcloc = Srcloc::start(&"*python*".to_string());
    val.as_ref(py).downcast::<PyList>().ok().map(|l| {
        if l.len() == 0 {
            ...
            for i_rev in 0..l.len() {
                ...
                let any_of_elt = map_err_to_pyerr(
                    srcloc.clone(), item
                )?;
                result = SExp::Cons(
                    srcloc.clone(),
        ...
        val.as_ref(py).downcast::<PyTuple>().map(|t| {
            if t.len() != 2 {
                ...
                let any_of_e0 = map_err_to_pyerr(
                    srcloc.clone(),
                    t.get_item(0).extract()
                )?;
                let any_of_e1 = map_err_to_pyerr(
                    srcloc.clone(),
        ...
        val.as_ref(py).downcast::<PyBytes>().map(|b| {
            Ok(Rc::new(SExp::Atom(srcloc.clone(), b.as_bytes().to_vec())))
        }).ok()
    }).map(|x| Some(x)).unwrap_or_else(|| {
        let stringified = format!("{}", val);
        stringified.parse::<BigInt>().map(|i| {
            if i == bi_zero() {
                Ok(Rc::new(SExp::Nil(srcloc.clone())))
            } else {
...
pub fn clvm_value_to_python(
    py: Python, val: Rc<SExp>
) -> Py<PyAny> {
    val.proper_list().map(|lst| {
        ...
        match val.borrow() {
            ...
            SExp::Integer(_,i) => {
                let int_val: Py<PyAny> = map_err_to_pyerr(
                    val.loc(),
                    py.eval(&format!("int({})", i), None, None).map(|x| x.into_py(py))
                ).unwrap();
                int_val
            },
