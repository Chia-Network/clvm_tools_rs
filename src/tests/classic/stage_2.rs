...
impl TestCompilerOptsPresentsOwnFiles {
    fn new(opts: Rc<dyn CompilerOpts>, files: HashMap<String, String>) -> Self {
        TestCompilerOptsPresentsOwnFiles { opts, files }
    }
}
...
impl HasCompilerOptsDelegation for TestCompilerOptsPresentsOwnFiles {
    ...
    fn override_write_new_file(&self, _target: &str, _content: &[u8]) -> Result<(), CompileErr> {
...
fn test_classic_compiler_with_compiler_opts() {
    ...
    let test_file = "test.clsp";
    let def_opts = Rc::new(DefaultCompilerOpts::new(test_file));
    let opts = Rc::new(TestCompilerOptsPresentsOwnFiles::new(def_opts, files));
