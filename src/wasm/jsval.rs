...
pub fn js_from_location(l: Srcloc) -> JsValue {
    ...
    loc_array.set(0, js_pair(JsValue::from_str("file"), JsValue::from_str(&file_copy)));
    loc_array.set(1, js_pair(JsValue::from_str("line"), JsValue::from_f64(l.line as f64)));
    loc_array.set(2, js_pair(JsValue::from_str("col"), JsValue::from_f64(l.col as f64)));
    match l.until {
        Some((tl,tc)) => {
            let til_array = Array::new();
            til_array.set(0, js_pair(JsValue::from_str("line"), JsValue::from_f64(tl as f64)));
            til_array.set(1, js_pair(JsValue::from_str("col"), JsValue::from_f64(tc as f64)));
            loc_array.set(3, object_to_value(Object::from_entries(&til_array).as_ref().unwrap()));
        },
        _ => { }
    }
...
pub fn js_object_from_sexp(v: Rc<SExp>) -> JsValue {
    match v.borrow() {
        ...
        SExp::Integer(_,i) => {
            JsValue::bigint_from_str(&i.to_string())
        },
        ...
        SExp::Cons(l,a,b) => {
            v.proper_list().map(|lst| {
                ...
                let array = Array::new();
                array.set(0, js_pair(JsValue::from_str("location"), js_from_location(l.clone())));
                let pair: JsValue = js_pair(JsValue::from_str("pair"), js_pair(
...
pub fn get_property(o: &Object, name: &str) -> Option<JsValue> {
    Object::try_from(&Object::get_own_property_descriptor(o, &JsValue::from_str(name))).
        and_then(|desc_obj| {
            match Object::try_from(desc_obj) {
                Some(o) => {
...
fn location_lc_pair(o: &Object) -> Option<(usize, usize)> {
    get_property(o, "line").and_then(|l| l.as_f64()).
        map(|l| (
            l as usize,
            get_property(o, "col").
                and_then(|c| c.as_f64()).map(|c| c as usize).unwrap_or_else(|| 0)
        ))
}

fn location(o: &Object) -> Option<Srcloc> {
    Object::try_from(o).and_then(|lo| {
        get_property(lo, "file").and_then(|f| f.as_string())
    }).and_then(|f| {
        location_lc_pair(o).map(|(l,c)| (f,l,c))
    }).map(|(f,l,c)| {
        Srcloc {
            file: Rc::new(f),
            line: l,
            col: c,
            until: get_property(o, "until").and_then(|lo| Object::try_from(&lo).map(|o| o.clone())).and_then(|lo| location_lc_pair(&lo))
        }
    })
}
