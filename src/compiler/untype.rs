...
fn run_failure_to_eval_err(sexp: NodePtr, e: &RunFailure) -> EvalErr {
    match e {
        RunFailure::RunExn(l,t) => EvalErr(sexp, format!("{}: exception: {}", l.to_string(), t.to_string())),
    ...
    match sexp.borrow() {
        SExp::Cons(l,a,b) => {
            if let SExp::Atom(la,n) = a.atomize() {
                if n.len() == 1 {
                    if n[0] == 1 || n[0] == b'q' {
                        return Rc::new(SExp::Cons(l.clone(), Rc::new(SExp::Atom(la.clone(), vec![1])), b.clone()));
                    } else {
                        let op = choose_operator(la.clone(), &n).unwrap_or_else(|| a.clone());
                        return Rc::new(SExp::Cons(l.clone(), op, flatten_type_level_operators_call(b.clone())));
                    }
                    ...
                    return Rc::new(SExp::Cons(l.clone(), Rc::new(SExp::Atom(la.clone(), n.clone())), flatten_type_level_operators_call(b.clone())));
                }
            }
...
fn untype_definition(
    ...
    let arguments =
        if let Some(typed) = recover_arg_type(converted[offset].clone(), false).map_err(|e| EvalErr(l, format!("{}: {}", e.0.to_string(), e.1)))? {
            typed.stripped_args
        } else {
    ...
    output_list.push(flatten_type_level_operators(converted[converted.len() - 1].clone()));
...
pub fn untype_code(
    ...
    if let Some(l) = proper_list(allocator, sexp, true) {
        ...
        for c in l.iter() {
            converted.push(convert_from_clvm_rs(allocator, loc.clone(), *c).map_err(|e| run_failure_to_eval_err(*c, &e))?);
        }

        if let Some(reformed) = matches_mod(opts.clone(), loc.clone(), sexp, &converted)? {
            debug!("reformed {}", reformed.to_string());
            return convert_to_clvm_rs(allocator, reformed).map_err(|e| run_failure_to_eval_err(sexp, &e));
        }
    };
