...
use crate::compiler::comptypes::{
    Binding, BodyForm, CompileErr, CompileForm, CompilerOpts, EvalResult, HelperForm, LetFormKind,
};
...
fn create_argument_captures(
    ...
    match (formed_arguments, function_arg_spec.borrow()) {
        (_, SExp::Nil(_)) => EvalResult::Continuable(()),
...
fn build_argument_captures(
    ...
    args: Rc<SExp>,
) -> EvalResult<HashMap<Vec<u8>, Rc<BodyForm>>, CompileErr> {
...
impl Evaluator {
    ...
    fn invoke_primitive(
        ...
        if call_name == &"@".as_bytes().to_vec() {
            ...
            self.lookup_prim(l.clone(), call_name)
                .map(|prim| -> EvalResult<Rc<BodyForm>, CompileErr> {
    ...
    fn handle_invoke(
        ...
        match helper {
            Some(HelperForm::Defconstant(_, _, _)) => EvalResult::Err(CompileErr(
                call_loc.clone(),
        ...
        match body.borrow() {
            ...
            BodyForm::Value(SExp::Atom(l, name)) => {
                if name == &"@".as_bytes().to_vec() {
                    let literal_args =
                        EvalResult::from_result(synthesize_args(prog_args.clone(), env))?;
                    self.shrink_bodyform(
                        allocator,
                        prog_args.clone(),
                        env,
                    ...
                    env.get(name)
                        ...
                        .unwrap_or_else(|| {
                            self.get_constant(name)
                                ...
                                .unwrap_or_else(|| {
                                    EvalResult::Final(Rc::new(BodyForm::Value(SExp::Atom(
                                        l.clone(),
                                        name.clone(),
                                    ))))
                                })
            ...
            BodyForm::Call(l, parts) => {
                if parts.len() == 0 {
                    return EvalResult::Err(CompileErr(
                        l.clone(),
                        format!("Impossible empty call list"),
                    ));
                }
    ...
    fn compile_code(
        ...
        let com_result = EvalResult::from_result(updated_opts.compile_program(
            allocator,
            self.runner.clone(),
            use_body,
            &mut HashMap::new(),
