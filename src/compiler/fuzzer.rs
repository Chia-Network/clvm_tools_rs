...
const BINDING_NAME_MAX: usize = 8;
...
fn random_args<R: Rng + ?Sized>(rng: &mut R, loc: Srcloc, a: ArgListType) -> SExp {
    match a {
        ...
        ArgListType::ProperList(n) => {
            ...
            let generated_args: Vec<Rc<SExp>> =
                (0..n).map(|_| Rc::new(rng.gen())).collect();
            enlist(loc, &generated_args)
        }
        ...
        match alternative {
            ...
            _ => {
                ...
                let new_bindings: Vec<FuzzBinding> = (1..=bound)
                    .map(|_| FuzzBinding {
                        name: random_atom_name(rng, BINDING_NAME_MIN, BINDING_NAME_MAX),
...
fn random_arglist_cons<R: Rng + ?Sized>(rng: &mut R, loc: &Srcloc, remaining: usize) -> SExp {
    if rng.gen() || remaining < 1 {
        SExp::Atom(loc.clone(), random_atom_name(rng, 2, 8))
    } else {
...
pub fn make_random_u64_seed() -> u64 {
    ...
    let random_seed = random_atom_name(&mut rng, 10, 10);
