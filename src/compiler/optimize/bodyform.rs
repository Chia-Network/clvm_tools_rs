...
fn visit_detect_in_body_form_inner<F,R,E>(
    ...
    f: &F,
    original: &BodyForm,
    bf: &BodyForm) -> Result<(), E>
where F: Fn(&[BodyformPathArc], &BodyForm, &BodyForm) -> Result<Option<R>,E> {
    ...
    match bf {
        ...
        BodyForm::Let(k, b) => {
            for (i, a) in b.bindings.iter().enumerate() {
                ...
                visit_detect_in_body_form_inner(path, res, f, original, b.body.borrow())?;
                path.truncate(path_idx);
            }
        }
...
pub fn visit_detect_in_body_form<F,R,E>(f: &F, bf: &BodyForm) -> Result<Vec<PathDetectVisitorResult<R>>, E>
where F: Fn(&[BodyformPathArc], &BodyForm, &BodyForm) -> Result<Option<R>,E> {
    let mut path = vec![];
    let mut res = vec![];
    ...
    F: Fn(&PathDetectVisitorResult<R>,&BodyForm) -> BodyForm
{
    ...
    for (i,a) in list_of.iter().enumerate() {
        ...
        let pass_on_replacements: Vec<PathDetectVisitorResult<R>> =
            replacements.iter().filter(|r| {
                path_overlap(&current_path, &r.path)
            }).cloned().collect();
        ...
        collection.push(
            compose_wrap(&a, replace_in_bodyform_subset(
                current_path, &pass_on_replacements, &extract_body(&a), f
            ))
        );
        current_path.truncate(path_idx);
    }
    ...
    F: Fn(&PathDetectVisitorResult<R>, &BodyForm) -> BodyForm
{
    ...
    let new_lambda_body = replace_in_bodyform_subset(
        current_path, &pass_on_replacements, &inner_body, f
    );
    current_path.truncate(path_idx);
    ...
    F: Fn(&PathDetectVisitorResult<R>, &BodyForm) -> BodyForm
{
    ...
    let exact_match_replacements: Vec<PathDetectVisitorResult<R>> =
        replacements.iter().filter(|r| {
            &r.path == current_path
        }).cloned().collect();
    ...
    match bf {
        ...
        BodyForm::Let(k, b) => {
            replace_in_bodyform_inner_list(
                ...
                &|w: &Rc<Binding>, b: BodyForm| {
                    ...
                    Rc::new(Binding { body: Rc::new(b), .. wb.clone() })
                },
                &|bindings| BodyForm::Let(k.clone(), Box::new(LetData {
                    bindings,
                    .. *b.clone()
                })),
                f
        ...
        BodyForm::Lambda(l) => {
            replace_in_bodyform_inner_body(
                current_path,
                &replacements,
        ...
        BodyForm::Mod(l, m) => {
            replace_in_bodyform_inner_body(
                ...
                &m.exp,
                &|b| {
                    BodyForm::Mod(l.clone(), CompileForm {
                        exp: Rc::new(b),
                        .. m.clone()
                    })
                },
...
pub fn replace_in_bodyform<F,R>(replacements: &[PathDetectVisitorResult<R>], bf: &BodyForm, f: &F) -> Option<BodyForm>
where
    R: Clone,
    F: Fn(&PathDetectVisitorResult<R>, &BodyForm) -> BodyForm
{
    ...
    Some(replace_in_bodyform_subset(&mut current_path, replacements, bf, f))
}
