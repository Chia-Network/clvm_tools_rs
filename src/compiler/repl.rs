...
use crate::compiler::comptypes::{CompilerOpts, CompileErr, BodyForm};
...
impl Repl {
    pub fn new(opts: Rc<dyn CompilerOpts>, runner: Rc<dyn TRunProgram>) -> Repl {
        ...
        let start_program_fe = frontend(
            opts.clone(),
            vec![starter_empty_program.clone()]
        ).unwrap();
    ...
    pub fn process_line(
        ...
        if self.depth < 0 {
            ...
            let result = parse_sexp(loc, &input_taken).map(|_v| {
        ...
        parse_sexp(self.loc.clone(), &input_taken).map_err(|e| {
            return CompileErr(e.0.clone(), e.1.clone());
