...
function rngu32(rng: ChaChaRng): number {
    return rng.nextU32();
}
...
const generateBasicValues = [
    (rng: ChaChaRng, lv: number) => nilValue,
    (rng: ChaChaRng, lv: number) => new FixedValue(
        0,
        "80",
        new Uint8Array([
            0x4b, 0xf5, 0x12, 0x2f, 0x34, 0x45, 0x54, 0xc5, 0x3b, 0xde, 0x2e, 0xbb, 0x8c, 0xd2, 0xb7, 0xe3, 0xd1, 0x60, 0x0a, 0xd6, 0x31, 0xc3, 0x85, 0xa5, 0xd7, 0xcc, 0xe2, 0x3c, 0x77, 0x85, 0x45, 0x9a
...
class RandomTuple implements ConvertibleValue {
    ...
    constructor(rng: ChaChaRng, lv: number) {
...
const valueGenerators = [
    (rng: ChaChaRng, lv: number) => new RandomS32Generator(rng),
    (rng: ChaChaRng, lv: number) => new RandomBigIntGenerator(rng),
    (rng: ChaChaRng, lv: number) => new RandomString(rng),
    (rng: ChaChaRng, lv: number) => new RandomBytes(rng),
];

const compositeGenerators = [
    (rng: ChaChaRng, lv: number) => new RandomTuple(rng, lv),
    (rng: ChaChaRng, lv: number) => new RandomList(rng, lv)
];
