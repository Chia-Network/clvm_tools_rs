...
var clvm_tools_rs = require('clvm_tools_rs');
...
clvm_tools_rs.then((clvm_tools_rs) => {
    function do_simple_run() {
        let value = run_program(clvm_tools_rs, fact_hex, [5], fact_sym, {
            "fact-base": (env) => {
    ...
    function do_complex_run() {
        ...
        let pwcoin = clvm_tools_rs.compile(pwcoin_src, 'pwcoin.cl', []);
        if (pwcoin.error) {
            throw new Error(pwcoin.error);
        }

        let value = run_program(clvm_tools_rs, pwcoin.hex, ["hello", bytestring("5f5767744f91c1c326d927a63d9b34fa7035c10e3eb838c44e3afe127c1b7675"), 2], pwcoin.symbols, {});
        if (value.length != 1) {
        ...
        let run_check_password = clvm_tools_rs.compose_run_function(
            pwcoin.hex,
            pwcoin.symbols,
            "check-password"
        );
        ...
        let not_right_password = run_program(
	          clvm_tools_rs, run_check_password, ["not-right"], pwcoin.symbols, {}
        );
        ...
        let right_password = run_program(
            clvm_tools_rs, run_check_password, ["hello"], pwcoin.symbols, {}
        );
        if (!right_password) {
