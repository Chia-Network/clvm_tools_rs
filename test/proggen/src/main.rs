...
fn do_program(db: &mut sqlite::Connection, dialect: Rc<SExp>, buf: &[u8]) -> Rc<SExp> {
...
fn write_prog_to_db(db: &mut sqlite::Connection, buf: &[u8], sexp: Rc<SExp>) {
    ...
    let mut stmt = db.prepare("insert or ignore into programs (b, t) values (?, ?)").expect("should work");
    let binds: &[sqlite::Value] =
        &[sqlite::Value::String(encoded), sqlite::Value::String(pts)];
...
fn main() {
    ...
    if let Err(_) = conn.execute("create table if not exists programs (b text primary key, t text)") {
    ...
    let sexp = do_program(&mut conn, dialect.clone(), &buf);
    write_prog_to_db(&mut conn, &buf, sexp);

    for p in 0..200000 {
        // Read a random row.
        {
            let mut stmt = conn.prepare("SELECT b as b FROM programs ORDER BY RANDOM() LIMIT 1;").expect("should have selectable stuff");

            if let Ok(sqlite::State::Row) = stmt.next() {
                let this_buf_repr = stmt.read::<String, _>("b").expect("should have the right column");
                buf = general_purpose::STANDARD.decode(this_buf_repr).expect("should exist");
            }
        }
